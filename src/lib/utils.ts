...
const devicePerformance =
  typeof window !== 'undefined' ? getDevicePerformanceLevel() : 'medium';
...
export function processImageUrl(originalUrl: string): string {
  ...
  if (
    ...
    originalUrl.includes('bgm.tv/pic')
  ) {
    const { proxyType: bangumiProxyType, proxyUrl: bangumiProxyUrl } =
      getBangumiImageProxyConfig();
    switch (bangumiProxyType) {
      ...
      case 'custom':
        if (bangumiProxyUrl)
          return `${bangumiProxyUrl}${encodeURIComponent(originalUrl)}`;
