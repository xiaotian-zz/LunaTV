    ...
    if (m3u8Str.includes('#EXT-X-KEY')) {
      ...
      if (task.aesConf.uri) {
        ...
        try {
          ...
          task.aesConf.key = keyArrayBuffer;
        } catch (error) {
          clearTimeout(keyTimeoutId);
          throw error;
        }
...
/**
 * 十六进制字符串转字节数组（HLS IV 格式如 "0x0123..."）
 */
...
/**
 * AES 解密（Web Crypto API，AES-CBC / PKCS7）
 */
export async function aesDecrypt(
  ...
  if (!key || key.byteLength === 0) return data;

  const ivBytes = iv ? hexToBytes(iv) : new Uint8Array(16);
