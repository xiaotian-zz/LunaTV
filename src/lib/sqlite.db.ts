...
export class SqliteStorage implements IStorage {
  ...
  // ==================== 登录日志 ====================
  async addLoginLog(loginLog: LoginLog): Promise<void> {
    this.db
      .prepare(
        'INSERT OR REPLACE INTO login_logs (id, value, login_time, created_at) VALUES (?, ?, ?, ?)',
      )
      .run(
        loginLog.id,
    ...
    this.db
      .prepare('DELETE FROM login_logs WHERE login_time < ?')
      .run(thirtyDaysAgo);
  }

  async getLoginLogs(limit = 100): Promise<LoginLog[]> {
    const rows = this.db
      .prepare('SELECT value FROM login_logs ORDER BY login_time DESC LIMIT ?')
      .all(limit) as Array<{ value: string }>;
    return rows.map((r) => JSON.parse(r.value) as LoginLog);
  }

  async clearLoginLogs(): Promise<void> {
    this.db.prepare('DELETE FROM login_logs').run();
    console.log('[SQLite] 已清除所有登录日志');
  }

  async getLastLoginLog(username: string): Promise<LoginLog | null> {
    const rows = this.db
      .prepare(
        'SELECT value FROM login_logs ORDER BY login_time DESC LIMIT 100',
      )
      .all() as Array<{ value: string }>;
    for (const row of rows) {
      const log = JSON.parse(row.value) as LoginLog;
