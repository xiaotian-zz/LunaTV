...
export async function POST(req: NextRequest) {
  try {
    ...
    // 步骤1：按原值恢复所有用户（V1 凭据直写不二次哈希 + V2 用户列表登记，
    //        避免 getAllUsers 在 users:list 非空后遗漏 V1 用户导致"用户丢失"）
    const userData = importData.data.userData;
    for (const username in userData) {
      const user = userData[username];
      const v2 = user.userInfoV2;
      try {
        await db.importUser(
          username,
        ...
        console.error(`恢复用户 ${username} 失败:`, e);
      }
    }
    ...
    for (const username in userData) {
      ...
      if (user.reminders) {
        for (const [key, reminder] of Object.entries(user.reminders)) {
          ...
          await (db as any).storage.setReminder(
            username,
            key,
            upgradedReminder,
          );
