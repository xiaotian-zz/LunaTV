    ...
    const initPlayer = async () => {
      ...
      try {
            ...
            },
            {
              ...
              tooltip: (() => {
                const opacity = parseFloat(
                  localStorage.getItem('control_bar_opacity') || '0.5',
                );
              ...
              onChange: function (item: any) {
                ...
                // 实时应用透明度到毛玻璃容器
                const liquidGlass = document.querySelector(
                  '.art-liquid-glass',
                ) as HTMLElement;
                if (liquidGlass) {
                  // 调整背景色透明度
        ...
        artPlayerRef.current.on('fullscreen', (isFullscreen: boolean) => {
          ...
          // 应用保存的透明度设置
          const liquidGlass =
            artPlayerRef.current?.template?.$player?.querySelector(
              '.art-liquid-glass',
            ) as HTMLElement | null;
          if (liquidGlass) {
            const savedOpacity = parseFloat(
            ...
            if (isFullscreen) {
              ...
              liquidGlass.style.setProperty(
                'background-image',
                `linear-gradient(to top, rgba(0, 0, 0, ${savedOpacity}), rgba(0, 0, 0, ${savedOpacity * 0.6}), transparent)`,
                'important',
              );
              liquidGlass.style.setProperty(
                'box-shadow',
                `0 -10px 30px rgba(0, 0, 0, ${savedOpacity * 0.8})`,
                'important',
              );
            } else {
              // 非全屏：恢复毛玻璃效果
