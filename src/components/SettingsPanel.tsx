...
const bufferModeOptions = [
  {
    ...
    label: '默认模式',
    description: '45秒缓冲+2路预取，适合网络稳定的环境',
    icon: '🎯',
    color: 'green',
  },
  {
    ...
    label: '增强模式',
    description: '90秒缓冲+3路预取，适合偶尔卡顿的网络环境',
    icon: '⚡',
    color: 'blue',
  },
  {
    value: 'max' as const,
    label: '强力模式',
    description: '135秒缓冲+4路预取，起播稍慢但播放更流畅',
    icon: '🚀',
    color: 'purple',
  },
];
...
export const SettingsPanel = memo(({ isOpen, onClose }: SettingsPanelProps) => {
  ...
  const [bangumiImageProxyType, setBangumiImageProxyType] =
    useState('cmliussss');
  ...
  useEffect(() => {
    ...
    setBangumiImageProxyType(
      localStorage.getItem('bangumiImageProxyType') ??
        RC.BANGUMI_IMAGE_PROXY_TYPE ??
        'cmliussss',
    );
    setBangumiImageProxyUrl(
      readLS('bangumiImageProxyUrl', RC.BANGUMI_IMAGE_PROXY || ''),
    );
  ...
  const handleBangumiApiTypeChange = (v: string) => {
    setBangumiApiType(v);
    localStorage.setItem('bangumiApiType', v);
  };
  const handleBangumiApiProxyChange = (v: string) => {
    setBangumiApiProxy(v);
    localStorage.setItem('bangumiApiProxy', v);
  };
  const handleBangumiImageProxyTypeChange = (v: string) => {
    setBangumiImageProxyType(v);
    localStorage.setItem('bangumiImageProxyType', v);
  };
  const handleBangumiImageProxyUrlChange = (v: string) => {
    setBangumiImageProxyUrl(v);
    localStorage.setItem('bangumiImageProxyUrl', v);
  };
  ...
  const handleResetSettings = () => {
    ...
    localStorage.setItem(
      ...
      RC.BANGUMI_IMAGE_PROXY_TYPE || 'cmliussss',
    );
                  ...
                  <div className='absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-auto'>
                    {bangumiApiTypeOptions.map((option) => (
                      <button
                        ...
                        type='button'
                        onClick={() => {
                          handleBangumiApiTypeChange(option.value);
                ...
                >
                  {
                    bangumiImageProxyTypeOptions.find(
                      (o) => o.value === bangumiImageProxyType,
                    )?.label
                  }
                </button>
                <div className='absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none'>
            ...
            {bangumiImageProxyType === 'custom' && (
              <div className='space-y-3'>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleBangumiImageProxyUrlChange(e.target.value)
                  }
                />
              </div>
            )}
